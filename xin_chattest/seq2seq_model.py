...
class Seq2SeqModel(object):
  ...
  def __init__(self, question_vocab_size, answer_vocab_size, buckets, size,
    ...
    Args:
      question_vocab_size: size of the question vocabulary.
    ...
    if num_samples > 0 and num_samples < self.answer_vocab_size:
      w = tf.get_variable("proj_w", [size, self.answer_vocab_size])
      w_t = tf.transpose(w)
      b = tf.get_variable("proj_b", [self.answer_vocab_size])
      output_projection = (w, b)

      def sampled_loss(labels,inputs):
        labels = tf.reshape(labels, [-1, 1])
        return tf.nn.sampled_softmax_loss(w_t, b, labels,inputs,num_samples,self.answer_vocab_size)
    ...
    for i in xrange(buckets[-1][1] + 1):
      ...
      self.answer_weights.append(tf.placeholder(tf.float32, shape=[None],
                                                name="weight{0}".format(i)))

    # Our answers are decoder inputs shifted by one.
    answers = [self.decoder_inputs[i + 1]
               for i in xrange(len(self.decoder_inputs) - 1)]

    # Training outputs and losses.
    if forward_only:
      self.outputs, self.losses = tf.contrib.legacy_seq2seq.model_with_buckets(
          self.encoder_inputs, self.decoder_inputs, answers,
          self.answer_weights, buckets, lambda x, y: seq2seq_f(x, y, True),
  ...
  def step(self, session, encoder_inputs, decoder_inputs, answer_weights,
    ...
    Args:
      ...
      answer_weights: list of numpy float vectors to feed as answer weights.
  ...
  def get_batch(self, data, bucket_id):
    ...
    for length_idx in xrange(decoder_size):
      ...
      # Create answer_weights to be 0 for answers that are padding.
      batch_weight = np.ones(self.batch_size, dtype=np.float32)
      for batch_idx in xrange(self.batch_size):
        # We set weight to 0 if the corresponding answer is a PAD symbol.
        # The corresponding answer is decoder_input shifted by 1 forward.
